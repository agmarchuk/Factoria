...
namespace FactographData
{
    public class RYEngine
    {
        ...
        public IEnumerable<RRecord> RSearch(string searchstring)
        {
            ...
            var res = fservice.SearchByName(searchstring)
        ...
        {
            var res = fservice.SearchByWords(searchstring)
                .Select(x => new RRecord
                {
                    Id = x.Attribute("id").Value,
                    Tp = x.Attribute("type").Value,
        ...
        {
            var item = fservice.GetItemByIdBasic(id, addinverse);
        ...
        {
            var res = fservice.PutItem(new XElement("delete",
        ...
        {
            fservice.PutItem(new XElement("substitute",
        ...
        public string NewRelation(string type, string inverseprop, string source)
        {
            ...
            var res = fservice.PutItem(new XElement(x1, new XElement(x2, new XAttribute("{http://www.w3.org/1999/02/22-rdf-syntax-ns#}resource", source)),
                new XAttribute("owner", User)));
        ...
        public void Update(RRecord record, string user)
        {
            var xres = new XElement(ToXName(record.Tp),
                    ...
                    new XAttribute("owner", User));
            var res = fservice.UpdateItem(xres);
            if (res.Name == "error") throw new Exception(res.Value);
        }
    }
}
