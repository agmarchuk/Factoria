...
Теперь сделаю выбор начального варианта языка... Немножко помучился, но сделал! Полное испытание 
...
Что-то "выскакивало" еще, надо записывать. Но начну с попытки сделать второй пункт.
...
Я уже начал было делать изменения в коде построителя Index.razor, но видимо надо начать с онтологии. И пора онтологии 
...
Пока остаются сомнения в рассматриваемой схеме. Дело в том, что могут возникнуть непредусмотренные
...
Похоже, накопилось многовато претензий. Надо кое-что снять. Возвращаюсь к коду. Все концентриуется
...
Вторая проблема заключается в том, как выходить в режим семьи и уходить в стандартный режим?
...
Итак, работаю на дайджестом папиных документов. 
...
Forms. Надо попробовать. И даже не MVC, а одностраничные приложения. Проведу-ка я некоторые эксперименты. 

### 20231118 09:48
Провел эксперименты, вроде получается. Сложнее всего было найти как пользоваться razor-компонентами в 
технологии Razor Pages. Смешно, но пришлось поискать. Теперь обдумываю идею провести эксперимент по пересобранию
проекта в технологии Razor Pages. Но тогда надо "убивать" несколько "зайцев". Кроме прямого эксперимента,
хорошо бы реализовать новый дизайн в технологии Material Design Lite. Как-нибудь по-проще. Кроме того,
хотелось бы правильно параметризовать разработку и все настройки увести в конфигурационный файл. Кроме того,
хотелось бы (заодно) написать учебный материал. А еще было бы замечательно написать пару научных статей...
Последнее, пока не "вырисовывается". Начну сначала.

Далее, будет текст учебного пособия. 

## Проектирование информационной системы в технологии ASP.NET (учебное пособие)
ASP.NET Core позволяет создавать Web-приложения и сервисы, используя язык программирования C# и язык разметки
HTML. Есть ряд направлений в этой технологии, это ASP.NET MVC, ASP.NET Razor Pages, Blazor и некоторые другие
"веточки". Они в основном интегрированы и могут использоваться в одном проекте. Мы будем учиться в
технологии Razor Pages. Она довольно проста и весьма эффективна. 

Перед тем, как начать, студенту рекомендуется почитать документацию фирмы Microsoft про основы технологии. Далее
есть смысл скачать ASP.NET Core SDK с сайта компании. Если Вам позволяют ресурсы Вашего компьютера,
скачайте Visual Studio 2022 или более позднюю. Если нет, или Вы работате в среде Linux, инструментарий
подбирайте самостоятельно, скорее всего, на начальном этапе Вам пригодится Visual Studio Code.

### Что мы будем проектировать?
Мы будем проектировать простую информационную (фактографическую) систему, построенную на принципах Semantic Web.
Не спешите пугаться, все не так уж страшно. Итак, поехали!

Почти любая информационная система строится по архитектуре "клиент-сервер". В системе есть клиентская 
программа и серверная программа или сервис или Web-приложение. Между ними осуществляется взаимодействие 
через протокол HTTP, когда клиент посылает (HTTP) запрос Request сервису, сервис выполняет запрос и возвращает
отклик Response. В нашем случае, мы находимся в Web-технологиях, особенностями которых является то, что
клиент является стандартным браузером (напр. Google Chrome) и то, что содержится в отклике Response - в
основном HTML-текст странички, которая будет следующей. Итак, через адресную строку или через форму или 
через гиперссылку формируется обращение к сервису, на что он отвечате новой страницей. 

Давайте попробуем сделать простейшее клиент-серверное построение через любой редактор. Сначала развернем ASP.NET Core SDK. 
Создадим пустую директорию и в ней с консоли запустим генерацию Razor-решения:
```
dotnet new razor
dir
```
В этой директории создадутся несколько файлов, вторая команда выюает список этих файлов. Вам не обязательно знать детали этих файлов и директорий,
Вам достаточно знать, что любой ASP.NET проект запускается командой 
```
dotnet watch
```
После исполнения, в Вашем браузере появится страница тестового сайта с приглашением Welcome! Информационная 
система создана. Заметьте, что консоль, откуда Вы запустили команду не вышла из команды. Это Ваше Web-приложение.
А страница, которая сформировалась - Ваш клиентский интерфейс. Теперь вы можете снова послать Request сервису,
напр. нажав обновление страницы и снова появится то же приглашение. Правда происходит это настолько быстро, 
что кажется, что страница не обновляется. Теперь выключим сервис тем, что на косоли нажмем <cntrl>C.
На клиенте вроде ничего не происходит, просто там "висит" все та же страница. Но если ее попытаться
обновить (F5 или кружочек в браузере), то появится информация об ошибке при обновлении. Это означает
что на запрос от браузера, клиент не получил ответа от (остановленного или уничтоженного) сервиса.

Теперь в той же консоли (настроенной на Вашу директорию), наберем 
```
notepad Program.cs
```
И мы увидим текст заускаемой программы. Ничего пока не понятно, но это не важно. Главное будем знать,
что редактируя файлы через любой текстовый редактор, мы можем изменять приложение. А потом снова запускать
его и работать с ним через браузер. 

Чтобы двигаться дальше, в Visual Studio или как-то подругому, породим аналогичный проект. В VS он создается
через шаблон Asp.net Core. Посмотрим через Обозреватель решений на порожденные папки и файлы. Сейчас мы
обратим внимание на папку Pages и ее содержимое. Если отбросить тонкосит, которые нам пока не нужны,
в папке Pages находятся отдельные страницы проектируемого сайта или информационной системы. Запустим решение 
через <cntrl>F5 или через меню Отладка/Запуск без отладки. Мы снова увидим созданную страницу. Если не увидим, 
то скорее всего, проект не имеет статуса запускаемого проекта и это легко исправить в обозревателе решений
через правую кнопку мыши. Теперь снова мы видим консоль, в которой запустилось и работает приложение и 
страницу браузера. Какая страница из тех, которые описаны в Pages запустилась? Это страница Index, именно
она запускается по умолчанию. Страница Index состоит из двух файлов: Index.cshtml и Index.cshtml.cs. Первый 
файл формирует страницу, второй обрабатывает пришедшщие от этой страницы запросы. Можно условно сказать:
frontend и backend. Остальные страницы устроены аналогично. Технология упрощенная, поэтому и тот и другой 
файл, в нашем случае, проектирует один и тот же разработчик. 

Откроем файл Index.cshtml в редакторе. Почти все понятно. Это HTML-текст части содержимого страницы. Изменив
Welcome на Привет убедимся, что мы можем создавать и модифицировать отдельные страницы. Начнем изучение
технологии через разработку простых, но важных решений. Сначала, уберем все лишнее, что отвлекает или 
мешает. Уберем файлы страниц Privacy и Error. Два файла из этой директории ViewImports.cshtml и ViewStart.html
имеют специальное значение. Первый перечисляет импортирование пространств имен, второй указывает на шаблон,
который используется как общая часть всех страниц построения. В указаный файл _Layout.cshtml мы заглянем 
и уберем лишнее. В этом файле уберем все, что плохо понимаем, уберем все link'и, уберем из body все, 
оставим только @RenderBody() - эта команда вставляет ту самую страницу, которая визуализируется
(Index). После всех модификаций, порожденная HTML страница будет попроще. С этого момента начнем разработку.

### Выдача парамтров формой и прием контроллером
Сначала научимся передавать данные из формы и принимать их и обрабатывать их контроллером. Будем называть части страниц
Имя.cshtml.cs контроллерами (название из MVC = Model-View-Controller). Вообще-то это объединение конроллера,
принимающего данные и модели, в которую принятые данные помещаются. 

Чтобы загрузить работой контроллер, страница должна иметь форму, через которую данные посылаются сервису. 
Уберем из Index.cshtml остатки html кода и вместо него создадим форму с единственным текстовым полем:
```
<form>
...
```
Испытаем это решение. В браузере появится форма ввода и кнопка. Наберем текст, напр. qwerty, нажмем на 
кнопку. Вроде ничего не произошло, но на самомделе форма послала Request, этот запрос обработался контроллером и
контроллер запустил построение новой страницы, снова появилась форма. Эту цепочку преобразований мы можем
увидеть во-первых в адресной строке, где набранный текст передается по правилам HTTP в виде комбинации:
```
https://localhost:7191/?inputstring=qwerty
```
То есть, появился вопросительный знак и появилась пара имя_параметра=значение_параметра. Если бы 
параметров было несколько, то и пар было бы несколько, а разделителем был бы символ &.

Мы предположили, что сформированный формой HTTP-запрос, главная часть которого - адресная строка, приходит 
на контроллер и там мог бы повлиять на формирование следующей страницы. Посмотрим внимательнее на 
контроллер, что мы там видим? Мы видим класс, имеющий конструктор и один метод. Логично предположить, 
что конструктор в какой-то момент (вначале) создает объект этого класса, а потом приход на контроллер
запроса, инициирует исполнение метода OnGet, только реакцию в этом методе надо бы описать. Сейчас эта
реакция пустая, вот запрос и не влияет на построение страницы. А как он может повлиять? Снова смотрим
на текст построителя Index.cshtml и мы видим во второй строке:
```
@model IndexModel
```
Это означает, что нам доступна структура модели, а модель - это объект контроллера. Проверим. Заведем 
в конроллере еще одно поле, напр. 
```
publiс string inputstring = "Одно поле";
```
а в построитель поместим распечатывание значения этого поля 
```
<div>
    @Model.inputstring
</div>
```
Испытание показывает, что путь работает. Только теперь надо "добыть" параметр и его использовать для
формирования модели. Для добывания параметра воспользуемся тем, что контроллер уже получил HttpRequest.
Так что теперь достаточно в контроллер добавить такую обработку:
```
    public void OnGet()
    {
        inputstring = this.Request.Query["inputstring"];
    }
```
И теперь после набора строки в форме, эта строка будет перемещаться под форму. 

Есть другой способ доставить значение от параметра к построителю. Через Binding. Для этого, достаточно
в модели, для нужного параметра, сделать пояснение:
```
[BindProperty(SupportsGet = true)]
```
Убираем оператор внутри OnGet и убеждаемся, что это работает. Последние модификации постротеля можно сделать
исходя из того, что в форму можно поместить предыдущее значение, это упростит модификацию текста при осуществения 
попыток напр. поиска или для более обширных форм, не будет требоваться перенабор полей при исправлении ошибки. 
Второй момент иногда экономит место на экране. Дело в том, что при вводе текстового поля, функция посылки 
submit выполняется переводом строки после набранного значения. В итоге, наш контроллер, будет:
```
...
```
А наш построитель может выглядеть:
```
@page
@model IndexModel
<form>
    <input type="text" name="inputstring" value="@Model.inputstring" />
</form>
<div>
    @Model.inputstring
</div>
```
Наверное, теперь все понятно.

### Подключение сервиса данных

### 20231119 06:16
Вчера отвлекся от переработки. А сегодня столкнулся с тем, что моя программа перестала работать. Крутил-вертел, понял, что она все же работает
только в режиме запуска из командной строки. Не понимаю... Все это укрепляет мое желание перестать "играться" с Блазором и вернуться к
надежным решениям базового Asp.net Core. Так что пока прекращаю работать над учебным пособием и начинаю перерабатывать проект из Блазора в 
Razor Pages. Сначала буду поменьше модифицировать и побольше воспроизводить. 

Сначала воспроизведу стили и общий шаблон. Сделал. Теперь сделаю поисковый интерфейс. Шаблон я сделал, теперь нужны данные. 

### 20231120 18:14
Я потратил много времени, чтобы разобраться с использованием компонентов в Razor Pages. Разобрался, тороплюсь записать самое важное, что
я понял и научился использовать. 

Первое - компоненты всегда (кроме случая программирования) предполагаются .razor - компонентами. Можно использовать их в Блазоре и Микрософт
в документации ВСЕГДА предполагает именно эту технологию, а можно использовать ее и в более "скромных" вариантах Asp.net Core, я попробовал
Razor Pages. Важно понимать, что разор-компонент это и иерархическое построение и некоторая функциональность, т.е. действия при определенных 
событиях. Иерархическое построение компонента можно развернуть через 
```
<component type="typeof(SypRazor.Components.Counter)" render-mode="Static" />
```
Где указан компонент (каунтер) и способ его использования, в данном случае статической развертки. Описанные динамические действия видимо
игнорируются. Другой (асинхронный) вариант рендеринга, вместе с подстановкой фактических параметров, приведен в следующем фрагменте:
```
@(await Html.RenderComponentAsync<Pager>(RenderMode.Static, 
        new { pageNumber = 400, totalRecords = 150, pageSize = 20, linkUrl = Url.PageLink("Index") }))
```
Если хочется вставлять razor-компоненты вместе с функциональностью, то надо запускать Blazor-машину. В программе-пускаче Program.cs надо 
добавить некоторые модификации, это добавить сервис и добавить что-то еще (кажется роутер), пример приведен ниже, причем добавляемые строки
закомментарены
```
...
//app.MapBlazorHub();
```
А еще надо добавить запуск блазоровского javascript, это делается в конце html-кода перед закрывающим </body>:
```
@*    <script src="_framework/blazor.server.js"></script>*@
```
Рекомендуются еще мелкие изменения, детали читай в статье https://ml-software.ch/posts/using-razor-components-in-razor-pages-or-mv-views , 
документации по рендерингу (для статического подхода) https://learn.microsoft.com/ru-ru/dotnet/api/microsoft.aspnetcore.mvc.rendering.htmlhelpercomponentextensions.rendercomponentasync?view=aspnetcore-7.0
и в статье https://www.mikesdotnetting.com/article/338/using-razor-components-in-a-razor-page
для динамического (RenderMode.Server) подхода. См. также документацию Microsoft: https://learn.microsoft.com/ru-ru/aspnet/core/blazor/components/prerendering-and-integration?view=aspnetcore-7.0&pivots=server

### 20231122 09:22
Похоже, я запутался в технологиях. Искал способ ПРОСТОГО формирования страниц, а вышел на сложности и непонимание. 
Возвращаюсь назад. Мне надо убидиться в том, что технология Razor Pages решает мои задачи по сайтостроительству. 
Я понял как можно делать и использовать razor-компоненты, но "сломался" на том, что не понял как формы помещать в компоненты и
их обрабатывать. Кажется, я сделал ошибку в том, что предпологал, что post-форму можно обрабатывать get-страницей.
Пусть есть протое get-построение, имеющее естественные параметры типа id, searchstring и т.д. Это построение может
создать post-форму. Но post-форма должна обрабатываться другим (post-)контроллером с итоговой переадресацией назад.

Попробую провести эксперимент. Создаю новый razor-проект, делаю в нем get-форму, заставляю работать. Потом
пытаюсь перенести эту форму в компонент и снова пытаюсь заставить работать. Если успешно, то делаю post-форму и
провожу те же исследования. 

Есть еще одно "магическое действие". В документации написано "Настройте доверие сертификату разработки HTTPS с помощью следующей команды:"
```
dotnet dev-certs https --trust
```
Очевидной реакции системы не видно, но при запуске приложения, перестает появляться предупреждение, что 
нельзя переадресовать по https. Нет, все же появляется: "Microsoft.AspNetCore.HttpsPolicy.HttpsRedirectionMiddleware[3]
Failed to determine the https port for redirect."

Первая часть эксперимента прошла хорошо. Get работает и в режиме статических компонентов. 

Вроде и post нормально работает. Некоторые неудобства доставляет то, что трудно совсем приблизить элементы 
цикла обработки. 

Для ясности, попробую представить работу такого компонента как SearchPanel. Компонент состоит из двух частей:
поля для образца и списка найденных вариантов. Одновременно в дереве портрета может быть несколько таких панелей.
Как не запутаться? Во-первых, мы отрабатываем поисковое нажатие. Контроллер вычислит варианты. И что? Очевидно,
что в полях формы должно находиться дополнительное поле, характиризующее место "нажатия". Возможно, это мог бы
быть идентификатор предиката прямой ссылки, которую мы обрабатываем. Если нет, то обрабатывается "верхняя" панель.
Если есть, то через построение находится подходящее место и туда вставляется и форма и результат поиска. А результаты 
поиска помещаются контроллером на какой-то верхний уровень. Видимо "верхний уровень" связан с корневой моделью
и должен передаваться следующим уровням через параметры. Компоненты здесь добавляют уровни и, наверное, среди параметров
должна быть ссылка на верхнюю модель. 

# Комментарии к проекту MaterialDesignHtml

Следую учебному материалу: https://coderlessons.com/tutorials/veb-razrabotka/izuchite-material-dizain-lite/obuchaiushchii-material-lite-lite-tutorial

Сначала делаю макет, "цепляющий" css и js файлы с сайта Google через Интернет. Начальный вариант в целом годится. Есть название
сайта, есть какое-то меню. Есть изменение композиции для узкого экрана.

### 20231125 14:53
Все же мне не дает покоя использование MDL для моих решений. Сегодня суббота (хотя уже и не утро...), попробую проделать эксперимент. 
В эксперименте я хочу собрать прототип или шаблон сайтового решения типа нынешнего SypBlazor. Что-ж, завожу html-файл Syp.html и
делаю там загрузки MDL.   

### 20231126 07:35
Верхнюю полосу я сделал. Получилось неплохо. Но попытка сделать tab-ы, не получиласть. Они оказались громоздкими. Решил теперь сделать
следующую полоску с двумя элементами по разную сторону. Слева будет поисковый интерфейс, справа будуь радио баттоны по управлению видом композиции. Попробую.

Сделал поисковую полосу. Теперь низ. Там меня более или менее все устраивает, за исключением: 
а) Квадратных портретиков; б) Увеличения фоток. Первое я переделаю на Card, второе - не знаю, может и так оставлю. Еще хорошо бы
использовать Badges. Может еще украсить кнопочки. Второе можно сделать через диалог, но там уже JavaScript... Еще надо модифицировать 
Tooltips.

Кажется, я придумал как изображать семью. Для этого надо иметь портретик вдвое большего размера (можно только по вертикали). Справа в первом ряду расположить братьев и сестер, во втором - жен/мужей. Сверху родители, снизу - дети. Как-то так...

Что-то я начинаю разочаровываться в Material Design Lite. Уменьшенные размеры я не использую, карточку сейчас "сбацал" без использования MDL, Везде поотказывался. Пожалуй только иконки это что-то полезное... Иконки как текст! Да еще и тень. Как-то это делается стандартными средствами. Посмотрел, вроде это что-то типа:
```
box-shadow: 0 2px 4px rgba(0, 0, 0, .2);
```
Можно и оставить MDL, надо посмотреть можно ли отключить JS. Можно, но тогда надо все события отрабатывать самостоятельно... Пожалуй, пока оставлю.

В общем, ни Tooltip ни Dialog меня не воодушевили, я решил, что без необходимости компоненты MDI использовать не буду. Но тогда надо
перевести все в нужный дизайн и в нужные стили. Действительно, похоже front-end разработка трудоемка...

### 20231127 13:14
Возвращаюсь к проекту. Я уже модернизировал используемый Box "ящик", улучшилось видение фотки, понятнее стал код
компонента. Теперь надо внести наработки по начальной полосе и по поисковому интерфейсу. 

Пока не буду переделывать "верхний" интерфейс, буду доделывать. Что мне надо доделать:
а) Спрятать перезагрузку
б) спрятать чекбокс и плюсик вокруг блока

Сделал, это было несложно. Теперь опубликую и проверю конфигурацию. 

### 20231128 21:48
В процессе работы над информационным наполнением, вижу, что не хватает упорядочивания по датам списков. Каких списков: 1) отражений; 2) элементов коллекции. Документы в многостраничных документах упорядочены по номерам страниц. Надо
найти это место и сделать более глубокое упорядочивание, но других списков.

Сортировку я сделал, теперь в списке убрать системную информацию из записей документов. Это надо делать и в
визуале и в редакторе. Но сначала, проще лишить статуса редактируемости эти поля не убирая их совсем.

Вроде и это сделал. Пока опубликую, надеюсь неожиданностей не возникнет.

### 20231216 16:24
Много времени занимался другими делами, теперь хочется вернуться к проекту. 

Что надо бы сделать? 
1 Первая страничка как-то дезориентирует. Может просто отказаться от нее? 
2 Очень неубедительно выглядят разделы семья и дерево. Можно попробовать семейную информацию показывать лишь для персон. 
3 Нужно по-новому оформить семью
4 Нужно расширить семейное дерево как было раньше, т.е. в сторону родителей. Тогда навигация будет непрерывной. 
5 Еще не шибко удобным явлется таблица пользователей, она публикуется, а значит - общая. 

Проблем много, надо что-то преодолевать. 

### 20231224 14:52
Надо доделывать. В прошлый раз я сделал пункты 2 и 3, 4-й пункт уже был ранее сделан. 5-й пункт
не имею идеи. Пожалуй, остался 1-й. Я просто убираю входую страницу и стартовую страницу прописываю в JSON-конфигураторе. 

### 20231228 08:00
Я все больше втягиваюсь в информационную работу. Но и работа над улучшением системы идет. Есть некоторые потребности:
1. Старая потребность - управлять набором свойств и классов, используемых при визуализации и редактировании;
2. Надо сделать оформление для коллекции через фотку, в которой коллекция "отражена".

Для управления набором свойств и классов, был разработан механизм тем. Надо его четко сформулировать и превратить в
"галочки" специализированной формы настройки. Важно, чтобы не было потери информации. А когда информация может потеряться?
Что означает потеря информации? Это означает исчезновение того что было. Мы можем что-то не визуализировать, но это не 
означает то, что это исчезло. Потеря информации может быть лишь если эта информмация находится в записи, а новая запись 
заместо старой "стирает" какие-то поля. Реально это означает, что папа и мама могут исчезнуть. Также как и даты, комментарии 
и др. Надо научиться "скрывать" от визуализации и редактирования указанные поля. По крайнем мере, это актуально для полей, 
которые ни в коем случае нельзя искажать ((uri, docmetainfo). Посмотрю код. 

### 20231229 13:16
Я подумал, что схема сокрытия предикатов должна быть разной. Для uri и docmetainfo важно, чтобы они были вычислены если
они есть. Для других - можно просто изъять из списка. Сначала позанимаюсь этими "другими". Пусть есть набор тем, соответствено,
набор "галочек". За каждой темой есть список предикатов, которые могут быть запрещены. Объединение отмеченных галочнами наборов даст общий
список. Все же попробую...

### 20231230 09:13
Вчера сделал решительный рывок в сторону управления темами. Кажется, можно реализовывать. Напрягаюсь, сделаю себе подарок на 
день рождения!

Немного запутался. Надо сохранить редактирование. Это значит, что редактируются все возможные поля, кроме uri и docmetainfo.
То есть эти поля должны сохраняться, но не изменяться. Возвращаюсь к работющему варианту и думаю как это сделать. 

Восстановил. Теперь правильно работает вплоть до редактирования и сохранения в базе данных. Теперь попробую внести управление
полями. 

Уфф... Вот я и сделал себе подарок. Еще вопросы остались, но это уже потом.

20231231 14:30
Последний день года! Я попытался заняться ТРИЗ-ассистентом. Вроде все понятно, но как это эффектно преподнести?
Не очень понятно как задействовать в проекте несколько школьников. Но первый вопрос - важнейший. Я уже думал в самых
разных направлениях. Например, про сказочные сюжета, может - Робинзон? Может подобрать изобретательские задачи
попроще?

Или набросать идей на тему транспорта будущего? Может не транспорта, может еще чего? Хорошо бы использовать ТРИЗ.
Может антидиверсионный анализ? Хорошо бы что-нибудь более нейтральное. Может сделать историю в стиле "Путь борьбы и побед"?

А насчет семейного сайта. Остались некоторые задумки. В частности:
- надо сделать сортировку по разделам семьи;
- надо бы сделать отражение коллекции в фотографии с использованием отражения в иконке коллекции; 
- надо сделать понятым наличие файла документа, который можно прочитать;
- были идеи вокруг прямого использования .txt и .html - файлов;

Начну с первого пункта. Первый пункт выполнил. Была простая ошибка. 

С некоторым напрягом, сделал и второй пункт. Третий пункт сделаю вставлением иконки документа и описания типа. 

Ура, успел сделать еще в 2023 году! Пора делать фиксацию и публикацию. 

### 20240101 07:40
С Новым годом! Вчера неплохо поработал. Осталась одна известная проблема, да и проблема ли? Это начальное установление
темами. Ну и темы пока не до конца сформированы. 

Сделал что хотел. Управление темами вроде работает. 
