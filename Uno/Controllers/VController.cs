...
public class VController : PageModel
{
    ...
    public void OnGet(HttpRequest request)
    {
        ...
        selectsbor = BuildSelectResults(db, ss, tp, bywords); // раздел выдачи результатов поиска
        portrait = BuildPortrait(db, id, shablons); // раздел портрета
    }

    private XElement BuildSelectResults(IFDataService db, string? ss, string? tp, bool bywords)
    {
        return !string.IsNullOrEmpty(ss) ? new XElement("div",
            ...
            new XElement("div", "== поиск завершен ==")) : new XElement("div");
            //.ToString() : "";
    }
    private Dictionary<string, string> Doctipnames()
    {
    ...
    private string BuildPortrait(Factograph.Data.IFDataService db, string id, Dictionary<string, Rec>? shablons)
    {
        ...
        if (id != null)
        {
            ...
            while (rrec != null) // Цикл вместо условия для выхода по break
            {
                ...
                else if (db.ontology.DescendantsAndSelf("http://fogid.net/o/document").Contains(tip))
                {
                    ...
                    string url = (tip == "http://fogid.net/o/photo-doc" ?
                        @Url.Content($"~/docs/GetImage?uri={uri}&size=medium") :
                        (tip == "http://fogid.net/o/video-doc" ?
                            @Url.Content($"~/docs/GetVideo?uri={uri}") :
                            (tip == "http://fogid.net/o/audio-doc" ?
                                @Url.Content($"~/docs/GetAudio?uri={uri}") :
    ...
    private string Kvad(string ttip, string iid, string name, string uri, string dates)
    {
        string url = "";
        if (ttip == "http://fogid.net/o/collection" || ttip == "http://fogid.net/o/cassette") url = Url.Content("/img/collection_m.jpg");
        else if (ttip == "http://fogid.net/o/photo-doc") url = Url.Content($"~/docs/GetImage?uri={uri}");
        else if (ttip == "http://fogid.net/o/video-doc") url = Url.Content("/img/video_m.jpg");
        else if (ttip == "http://fogid.net/o/audio-doc") url = Url.Content("/img/audio_m.jpg");
